...
class UniverseView {
    ...
    // Method 1: Apply filter of range
    applyRangeFilter(feature, minVal, maxVal) {
        // which songs to consider
        ...
        this.circles
            ...
            .attr('opacity', d => {
                ...
                if (this.currentSelection) {
                    ...
                    // is selected -> apply range filter
                    return (value >= minVal && value <= maxVal) ? 0.9 : 0.15;
                } else {
                    // not lasso selection -> normal filter
                    return (value >= minVal && value <= maxVal) ? 0.9 : 0.15;
                }
            })
            .attr('stroke-width', d => {
        ...
        // filter only inside the actual selection
        ...
        this.circles = this.zoomGroup.selectAll('circle.song-point')
            ...
            .on('mouseout', function(event, d) {
                ...
                d3.select(this)
                    .transition()
                    .duration(200)
                    .attr('opacity', targetOpacity)  // keep  actual opaque
                    .attr('stroke-width', currentOpacity > 0.8 ? 2 : 1)
                    .attr('stroke', 'white');  // turn to the white border
    ...
    drawMarginalDistributions() {
        ...
        const gradIdX = "grad-x-" + this.colorMode; // unique ID based on mode
    ...
    drawTopHistogram(height, numBins) {
        //Usar ticks for uniform bins 
        const xBins = d3.bin()
            .domain(this.xScale.domain())
            .thresholds(this.xScale.ticks(numBins)) 
            .value(d => d.tsne_1);
        ...
        const topHistGroup = this.g.append('g')
    ...
    drawRightHistogram(width, numBins) {
        const yBins = d3.bin()
            .domain(this.yScale.domain())
            .thresholds(this.yScale.ticks(numBins)) 
            .value(d => d.tsne_2);
        ...
        const rightHistGroup = this.g.append('g')  
    ...
    setupZoom() {
        ...
        this.zoom = d3.zoom()
            ...
            .filter(function(event) {
                ...
                // Pan con drag: only if lasso mode
                if (event.type === 'mousedown' || event.type === 'mousemove') {
                    return !self.lassoMode;  // ← Clave: verificar modo
                }

                // Touch events (móviles):only if not in lasso mode
